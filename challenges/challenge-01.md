...
**TASK.** Design $\Omega(t)$, $\delta(t)$ that prepare the Bell state $|\Psi^+\rangle = \left(|gr\rangle + |rg\rangle\right)/\sqrt{2}$ from $|gg\rangle$ at spacing $r_1$, then re-optimize at $r_2$.
...
(realized as 352 ns on the 4 ns clock; on hardware, the global channel's 8 MHz modulation bandwidth rounds the square edges — simulate with `with_modulation=True` to preview the realized pulse). At $\Omega_{\mathrm{ref}}$ the blockade radius is $R_b \approx 7.2\ \mu\mathrm{m}$, so both spacings are blockaded — but not equally:
...
**SCORE.** Bell-state fidelity $F = |\langle\Psi^+|\psi(T)\rangle|^2$ in simulation; hardware validation compares measured populations $P_{gg}, P_{gr}, P_{rg}, P_{rr}$ (recommended: 500 shots) against simulated values. Populations check the model, not the coherence — $F$ itself is scored in simulation.
